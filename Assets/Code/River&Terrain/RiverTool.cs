...
public class RiverTool : MonoBehaviour
{
        ...
        for (int i = 0; i < vertices.Count; i += 2)
        {
            if (i < vertices.Count && i-3 > -1)
            {
                ...
                if (tempNodes.Length > tempNodeIndex)
                    nodes.Add(new RiverNode(nodes.Count, vertices[i + 1], vertices[i], GetRiverFlow(last_1_pos, last_3_pos), tempNodes[tempNodeIndex].centerVectorOffset, tempNodes[tempNodeIndex].flowDirectionOffset));
                else
                    nodes.Add(new RiverNode(nodes.Count, vertices[i + 1], vertices[i], GetRiverFlow(last_1_pos, last_3_pos), Vector3.zero, Vector3.zero));
            ...
            else if(i + 2 < vertices.Count && i == 0)
            {
                ...
                if (tempNodes.Length > tempNodeIndex)
                    nodes.Add(new RiverNode(nodes.Count, vertices[i + 1], vertices[i], GetRiverFlow(last_3_pos, last_1_pos), tempNodes[tempNodeIndex].centerVectorOffset, tempNodes[tempNodeIndex].flowDirectionOffset));
                else
                    nodes.Add(new RiverNode(nodes.Count, vertices[i + 1], vertices[i], GetRiverFlow(last_3_pos, last_1_pos), Vector3.zero, Vector3.zero));
    ...
    public void AddStraitRow()
    {
        ...
        nodes.Add(new RiverNode(nodes.Count, vertices[vertices.Count - 1], vertices[vertices.Count - 2], dir_1, Vector3.zero, Vector3.zero));
    }
